...
namespace http {
    namespace server3 {
        ...
        std::vector<boost::asio::const_buffer> reply::to_buffers() {
            ...
            v.push_back(status_strings::to_buffer(status));
            for (auto &h : headers) {
                v.push_back(boost::asio::buffer(h.name));
                v.push_back(boost::asio::buffer(misc_strings::name_value_separator));
                v.push_back(boost::asio::buffer(h.value));
                v.push_back(boost::asio::buffer(misc_strings::crlf));
            }
            v.push_back(boost::asio::buffer(misc_strings::crlf));
            v.push_back(boost::asio::buffer(content));
