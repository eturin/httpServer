        ...
        bool Context::prepare(pqxx::connection *conn) {
            ...
            if (conn->is_open()) {
                ...
                insert into rq_ВходящаяОчередьid (Ссылка                                                ,ПометкаУдаления,ИмяПредопределенныхДанных,Дата ,URL,Метод,ТипСообщения,Размер,КоличествоПопыток,СообщениеОбработчика,ДатаОбработки               ,ВходящийИдентификатор,data)
                                          values (decode(replace(uuid_generate_v4()::text,'-',''),'hex'),false          ,''                       ,now(),$1 ,$2   ,$3          ,$4    ,0                ,''                  ,'0001-01-01 00:00:00.000000',''                   ,E'\\x01013D00000000000000EFBBBF7B2223222C38373132363230302D336539382D343465302D623933312D6363623164376564633439372C0D0A7B312C0D0A7B307D0D0A7D0D0A7D00000000'::bytea || $5 || E'\\x00000000'::bytea || $6)
                returning Ссылка;
                )");
                conn->prepare("dml_queue_get", R"(
                --запрос
                insert into rq_ВходящаяОчередьid (Ссылка                                                ,ПометкаУдаления,ИмяПредопределенныхДанных,Дата ,URL,Метод,ТипСообщения,Размер,КоличествоПопыток,СообщениеОбработчика,ДатаОбработки               ,ВходящийИдентификатор,data                                      )
                                          values (decode(replace(uuid_generate_v4()::text,'-',''),'hex'),false          ,''                       ,now(),$1 ,$2   ,$3          ,0     ,0                ,''                  ,'0001-01-01 00:00:00.000000',''                   ,E'\\x01010800000000000000EFBBBF7B2255227D')
                returning Ссылка;
                )");
